...
slack = new slackClient(process.env.SLACK_API_TOKEN, true)
      ...
      async.waterfall [
            ...
            if err
              ...
              while i <= data.members.length
                if data.members[i].name == name
                  return callback(null, data.members[i].id)
                else if i == data.members.length - 1 and data.members[i].name != name
                  callback 0, null
                i++
            return
          return
        (userID, callback) ->
          if userID == 0
            ...
            if room == 'trash-talk' or room == 'ayylmao' or room == 'afa' or room == 'gameplan'
              callback null, false, true
            else
              slack.channels.list (err, data) ->
                if err
                  ...
                  for o of data.channels
                    if data.channels[o].name == room
                      if _.contains(data.channels[o].members, userID)
              ...
              slack.groups.list (err, data) ->
                if err
                  ...
                  for p of data.groups
                    if data.groups[p].name == room
                      if _.contains(data.groups[p].members, userID)
                        return callback(null, false, true)
                      else
                        return callback(null, false, false)
                return
          return
      ], (err, userExistsInChannel, userExistsInGroup) ->
        if err
          message = err
        else if userExistsInChannel == false and userExistsInGroup == false
