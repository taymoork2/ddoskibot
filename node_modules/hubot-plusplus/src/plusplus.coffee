...
module.exports = (robot) ->
  ...
  exclude_from_harm = (name) ->
    exclude = ['zhi', 'shashank']
    for person in exclude
      if name.toLowerCase().indexOf(person) != -1
        return true
    false
  ...
  robot.hear ///
    ...
    \s*
    # the increment/decrement operator ++ or -- OR the add to/subtract to operator += or -=
    ([-+]{2}|—|[-+]=)
  ...
  ///i, (msg) ->
    ...
    [dummy, name, operator, delta, reason] = msg.match
    delta = parseInt(delta) #convert string to int
    ...
    if operator == "++"
      ...
      if exclude_from_harm(name)
        taunt = true
      else
        [score, reasonScore] = scoreKeeper.subtract(name, from, room, reason)
    else if operator == "+="
      [score, reasonScore] = scoreKeeper.addX(name, from, room, {reason: reason, x: delta})
    else
      if exclude_from_harm(name)
        taunt = true
      else
        [score, reasonScore] = scoreKeeper.subtractX(name, from, room, {reason: reason, x: delta})
    ...
    if taunt
      msg.send "haha nice try, #{name} is invincible"
