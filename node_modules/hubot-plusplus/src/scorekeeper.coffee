...
class ScoreKeeper
  ...
  addX: (user, from, room, options = {}) ->
    if @validate(user, from)
      user = @getUser(user)
      @storage.scores[user] += options.x
      @storage.reasons[user] ||= {}

      if options.reason
        @storage.reasons[user][options.reason] ||= 0
        @storage.reasons[user][options.reason] += options.x

      @saveUser(user, from, room, options.reason)
    else
      [null, null]
  ...
  subtractX: (user, from, room, options = {}) ->
    if @validate(user, from)
      user = @getUser(user)
      @storage.scores[user] -= options.x
      @storage.reasons[user] ||= {}
